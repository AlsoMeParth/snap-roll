...
from src.database.db import create_teacher, check_teacher_exist, teacher_login, get_teacher_subjects, get_attendance_for_teacher
    ...
    if st.session_state.attendance_images:
        ...
        for idx, image in enumerate(st.session_state.attendance_images):
            with gallery_cols[idx % 4]:
                st.image(image, width = 'stretch', caption = f'Photo {idx+1}')
        ...
        if st.button('Run Face Analysis', type = "secondary", width = "stretch", icon = ":material/analytics:", disabled = not has_photos):
            with st.spinner("Deep scanning classroom photos..."):
                ...
                for idx, img in enumerate(st.session_state.attendance_images):
                    ...
                    if detected:
                        for sid in detected.keys():  # maybe error (includes both 1 and 0)
                            student_id = int(sid)
...
def teacher_tab_attendance_records():
    ...
    for r in records:
        ...
        data.append({
            ...
            "Subject Code": r['subjects']['subject_id'],
...
def teacher_screen_register():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input(":gray[Enter username]", placeholder="royvishal")
    teacher_name = st.text_input(":gray[Enter Name]", placeholder="Vishal Roy")
    teacher_password = st.text_input(":gray[Enter password]", type="password", placeholder="Enter your password")
    teacher_password_confirm = st.text_input(":gray[Confirm Password]", type="password", placeholder="Re-enter password")
    st.divider()
    btncol1, btncol2 = st.columns(2)
    with btncol1:
